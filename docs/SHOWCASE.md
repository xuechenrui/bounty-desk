...
BountyDesk closes the payment loop for autonomous freelance and bounty agents. A ZeroClaw agent creates a uniquely referenced native SOL or SPL-token Solana Pay request from operator-approved terms, hands the unsigned URL to the payer, and later reconciles the payment through read-only Solana RPC. It emits a receipt only after deterministic Rust code verifies the reference, recipient, asset, and raw amount.

## Who it is for

- Open-source bounty contributors paid in SOL or Solana USDC.
...
Then run the valid and attack commands from the root README. The test suite includes nine passing cases and requires no wallet or network. To exercise live reconciliation, create an invoice with a Solana recipient, pay its URL from a wallet, then run `bounty-desk reconcile`.
...
2. **0:30–1:10** — Create a devnet native SOL invoice; show the unique reference and Solana Pay URL; point out that no secret exists.
